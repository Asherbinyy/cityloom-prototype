...
void main() {
  ...
  group('AppState and Card Unlock Logic', () {
    ...
    test('Burke unlocks ONLY when apprentice_q7 is answered correctly (not on q6)', () {
      final appState = AppState();
      ...
      expect(appState.isCardUnlocked('burke'), false); // Must NOT unlock on Q6

      appState.recordQuestionResult('apprentice_q7', true);
      expect(appState.isCardUnlocked('burke'), true); // Unlocks on Q7!
    });

    test('Hare unlocks when historian_q4 is answered correctly', () {
      final appState = AppState();
    ...
    test('Margaret unlocks when Burke, Hare, and scholar_q13 are correct', () {
      ...
      appState.recordQuestionResult('historian_q4', true); // unlocks Hare
      appState.recordQuestionResult('scholar_q13', true); // trigger (Odd one out)
